...
th.daemon = True  # 使用 daemon 属性替代 setDaemon()
th.start()
    ...
    if not __isRunning:  # 检测是否开启玩法，没有开启则返回原图像
        ...
        if not start_pick_up:
            ...
            if max_area > 2500:  # 有找到最大面积
                ...
                box = np.int32(cv2.boxPoints(rect))  # 使用 int32 替代已弃用的 int0
...
if __name__ == '__main__':
    import os
    # 设置 OpenCV 使用非 Qt 后端，避免在没有 X11 显示时的错误
    ...
    while True:
        ...
        if ret:
            ...
            print("无法从流服务器读取图像，请确保 MasterPi.py 正在运行")
            time.sleep(1)
